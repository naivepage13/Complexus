...
(function () {
    ...
    // Paleta do Complexus. O mapa nasceu num tema escuro; ao entrar no jogo
    // passou para a paleta clara, a mesma de css/app.css.
    const CORES = {
        estado: '#2f6fed',
        estadoArea: '#cfe0fd',
        cidade: '#d1495b',
        estrada: '#c77700',
        estradaBloqueada: '#d1495b'
    };
    ...
    //
    // A medida e refeita em alguns momentos pontuais, e nao por ResizeObserver:
    // observar o container faz o invalidateSize disparar durante as proprias
    // animacoes de zoom do Leaflet e cancela o zoom, deixando o mapa travado.
    ...
    function estiloEstado() {
        return { color: CORES.estado, weight: 1.5, fillColor: CORES.estadoArea, fillOpacity: 0.45 };
    }
    ...
    observarMudancas(() => {
        camadaEstradas?.camadasAtivas.forEach((linha, id) => {
            ...
            if (!estrada) return;
            linha.setStyle({ color: estrada.status === 'bloqueada' ? CORES.estradaBloqueada : CORES.estrada });
        });
    });
